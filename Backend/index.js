...
const sendEmail = async (config) => {
  ...
  const mailOptions = {
    from: config.email,
    to: process.env.DEFAULT_RECEIVER_EMAIL, // Using the receiver email from .env
    subject: 'Test Mail',
    html: `<h1>Test Email</h1><br><p>This is a warmup email. This email need reply${Date.now()}</p>`,
  };
...
const repliedEmail = async (email) => {
  const transporter = nodemailer.createTransport({
    ...
    secure: true,
    auth: {
      user: process.env.DEFAULT_RECEIVER_EMAIL, // Using the receiver email from .env
      pass: process.env.EMAIL_PASS, // Using email app password from .env
    },
  });

  const mailOptions = {
    from: process.env.DEFAULT_RECEIVER_EMAIL,
    to: email, // Preconfigured default email
    subject: 'Reply Email ',
    html: '<h1>Reply Email Test</h1> <br> <p>This is a warmup email.</p>',
  };
